...
def crear_solicitudes(cantidad_solicitudes):
    ...
    for n in range(1, cantidad_solicitudes + 1):
        ...
        if (hora_actual >= 1800):  
            hora_actual = 800
        contacto_cliente = "Cliente_" + (cliente_id)
...
def generar_reporte(asignaciones, cantidad_tecnicos):
    ...
    for i in range(cantidad_tecnicos):
        total_tareas = resueltos_por_tecnico + no_resueltos_por_tecnico
        if (total_tareas > 0):
            porcentaje_resueltas = (resueltos_por_tecnico / total_tareas) * 100
            porcentaje_no_resueltas = (no_resueltos_por_tecnico / total_tareas) * 100
